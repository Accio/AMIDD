...
years, drug discovery was primarily driven by serendipity and trials and errors
in chemistry, biology, and medicine. Today, drug discovery represents one of the
most interdisciplinary and collaborative human research enterprises. As the
techniques of drug discovery developed and refined, mathematical, statistical,
and computational models have become essential besides experimental methods.

We apply these models for two reasons mainly. First, they help us study the
...
to characterize the drug molecules and their interactions with the human body.
While it is perfectly feasible and sometimes necessary to ignore the human body
system and consider drug molecules only, we model drug-body interactions in
most cases because they form the mechanistic foundation of the therapeutic use of
drugs.
...
from 'disease' to 'health' is defined as its *efficacy profile*. Its capacity of
causing harm, known as *adverse effects*, to the whole system is defined as its
*toxicity profile*. Equivalently, its capacity of not causing adverse effects is
its *safety profile*. Therefore, models also reveal *efficacy and safety
...
appropriate prototypes of drug candidates. We collect data of the disease and
data of drug candidates from experiments. Models characterize diseases as states
of the system and encode our understanding of drug-body interactions. Informed
by these models, we use our expertise in chemistry and biology to change the
...
discovery. It can also be used by researchers working on disease understanding
and drug discovery as a practical guide to mathematical, statistical, and
computational modelling.

The book does not aim to cover the technicality. Instead, it attempts to offer a
broad overview of basic concepts and principles of quantitative modelling in
drug discovery.  References to resources dedicated to individual modelling
approaches are provided for interested readers.

Prerequisite is high-school knowledge of mathematics, chemistry, physics, and
biology. An appendix is available to refresh the essential knowledge in
mathematics, statistics, and programming that is used throughout the book.
...
mathematical models, statistical methods, and computational tools change our way
of finding drugs. 
...
inform each other to improve the affinity and specificity of a molecule
iteratively. We examine core concepts and techniques of structure-based
modelling for computer-aided drug design. We also discuss further applications
including toxicity prediction and virtual screening.

#### Chapter Eight: Ligand-based Modelling {-}

Complementary to structure-based modelling, ligand-based modelling uses
cheminformatics tools to describe chemical molecules and infer their
...
them. It is an important mathematical model that links molecule-level modelling
with cell-level modelling. We examine selected deterministic and probabilistic
...
Illumina(R) next-generation sequencing and Nanopore sequencing. We discuss key
concepts and models to analyse the sequencing data. And we examine their
applications in drug discovery. In particular, we will introduce single-cell
sequencing technologies and multimodal analysis, which hold promise for
knowledge about diseases and drug-body interaction on the level of individual
cells and interactions between the cells.
...
Biological imaging offers an approach orthogonal to omics technologies that were
introduced in the last two chapters to the study of biological processes.
Application of imaging spans a wide range between molecular modelling and
population modelling. 

Here, we first give a brief overview of imaging technologies used in drug
discovery. Next, we focus on cellular level imaging, in particular high-content
...
characterize PK and the PD profiles of drug candidates.
...
offer an integrative approach to combine preclinical mechanistic data with PK
and PD profiles and patient-specific characteristics collected in clinical
studies. They can identify mechanisms of actions of drugs, which may offer
insight for compound optimization or may even propose new targets. They can also
assess safety liabilities under different dosing regimens in different
populations quantitatively.

We introduce principles and basic applications of system models to investigate
either mechanism-of-action or safety liabilities of drug candidates. We will
learn from systems models developed in drug discovery and development
projects.
...
power calculation, randomization, blinding, and causality inference. We also
examine the common forms of RCT in drug discovery.
...
techniques, in particular the statistical model that is known as non-linear mixed-effect
modelling.
...
serve the development of the drug product for which the data were
collected. Recently, however, researchers realize the potential of such data to
inform preclinical discovery. This practice is summarized in the term *reverse
translation*. We examine key modelling approaches used in reverse translation as well as both
...
modelling in drug discovery. The examples come from both industrial and academic research. 

* Discovery of Vemurafenib
* Discovery of Sofosbuvir
* Discovery of Risdiplam
* Discovery of halicin as an new antibiotic
...
* On the organ and system level, better models to simulate human physiology,
  ...
  allow better predictions of PK and PD profiles. Patient-specific data and
  knowledge of genotype-phenotype association may allow individualized dosing.
* On the population level, digital biomarkers and integration of genotype and
  phenotype data may allow continuous and non-invasive characterization of
  response to drugs. 

We list key open questions about integrating models at different levels and discuss
potential solutions. We study the success of multiscale modelling in two other
...
may also help us make better decisions that are optimized given facts,
uncertainty, and risk. If implemented, these measures may revolutionize the way
we discover drugs in the future.
