...
This document is the preface to *Multiscale Modelling in Drug Discovery: An
Introduction* (working title), a book that I am writing. 
...
A mathematical model describes the state and dynamics of a system. A statistical
model represents assumptions about sample data. It allows us to use data
analysis to infer the distribution of variables of interest. A computational
model implements logic in procedures. It simulates the behaviour of a complex
system. There are different views in the literature of philosophy of science
about whether statistical and computational models are special forms of
mathematical models or they are distinct forms of conceptual modelling. For our
purpose though, it suffices that we refer to all of them as models and specify
the type only in case of ambiguity.

We apply models for two reasons mainly. First, they allow us study the human
...
from the Alzheimer's Disease, the lung infected by the coronavirus, and a
melanoma tissue, a skin cancer that originates from pigment-producing cells
...
The other application of models is to study body-drug interactions. While it is
feasible and sometimes helpful to ignore the human body system and model drug
molecules only, the effectiveness of such models will finally be tested in the
body system. Therefore, even if we start by modelling drug molecules only, we
will end up modelling body-drug interactions because they form the mechanistic
foundation of the therapeutic use of drugs.
...
*toxicity*. Equivalently, its capacity of not causing adverse effects is its
*safety*. Therefore, models characterizing body-drug interactions reveal
*efficacy and safety profiles* of drug candidates.

It is obvious that the two types of applications are dependent on each other:
better understanding of the human body system and more knowledge of diseases
catalyse new drugs with better efficacy and safety profiles. The study of
body-drug interactions offers more than chemical or biological tools as drugs
that treat diseases. The tools can be used as perturbations to interrogate
biological model systems of various kinds, including cultured cells, known as
*in vitro* systems, and animal models, known as *in vivo* systems. Such studies
may reveal unexpected aspects of diseases or even new insights into fundamental
biology.
...
3. drug modalities, such as small molecules, antibodies, and oligonucleotides.

#### Chapter Two: The Linear View of Drug Discovery {-}

The linear view of drug discovery is prevalent. It is found everywhere from
scientific publications to websites of pharma companies. In this chapter, we will
...
in order to treat diseases, must interact with and modulate components specified
in the central dogma either directly or indirectly.
...
these methods. Finally, we introduce databases and software tools that store,
...
specificity of a molecule iteratively. We examine concepts and techniques of
...
that connect the nodes. It is an important mathematical model that links
...
These analyses hold promise for knowledge about diseases and body-drug
interaction on the level of individual cells and between-cell interactions. 
...
optimization or may even lead to identification of new targets. They can also
...
inference. We also examine the common forms of RCT in drug discovery.
...
real-world data, like clinical-trial data, are analysed 'for-purpose' to
...
* At the cellular level, multimodal analysis is becoming a prominent tool to
  associate different aspects of cell biology and to integrate them into a
  unified view.
* At the organ and system level, better models to simulate human physiology,
  ...
  genotype-phenotype association accumulated in population studies may allow
  personalized PK and PD models and individualized dosing.
* At the population level, digital biomarkers and integration of real-world data
  may allow continuous and non-invasive characterization of response to drugs. 
...
molecular modelling in chemistry and material science in physics, and speculate
how drug discovery may learn and benefit from their approaches. 
...
I would like to thank many mentors, colleagues, and friends who have helped
