...
labelBG2 <- function (tr) {
    ...
    ## simple now: take two digits, later we can improve it
    rows <- gsub("([0-9]\\.[0-9][0-9])[0-9]+", "\\1", rows)
    
    boxed.labels(xy$x[ind], xy$y[ind] + 0.5 * charht, rows[ind], 
        border = F, bg = "white", cex = 0.8, xpad = 0.5, ypad = 1)
}
assignInNamespace("labelBG", labelBG2, "reprtree")
