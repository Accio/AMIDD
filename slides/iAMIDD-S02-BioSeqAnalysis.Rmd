...
*<1->The minimum number of operations required to transform string $a$ to string $b$ with following operations:
    * Insersion, for instance sit $\textrm{like}~\rightarrow~\textrm{alike}$ 
    * Deletion, \textit{e.g.} $\textrm{boat}~\rightarrow~\textrm{bot}$
    * Substitution, \textit{e.g.} $\textrm{pig}~\rightarrow~\textrm{big}$
*<2->The Levenshtein distance between two strings $a, b$ of length $|a|$ and $|b|$ respectively is given by $\operatorname{lev}_{a,b}(|a|,|b|)$ where
...
*<3->The Levenshtein distance is often referred to as the *edit distance*. It is often used in computational linguistics and natural language processing, for example in spellchecking.
